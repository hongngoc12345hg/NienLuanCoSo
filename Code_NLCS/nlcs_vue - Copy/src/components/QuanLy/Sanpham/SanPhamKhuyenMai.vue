<template>
  <div class="container-fluid text-center">
    <div class="container">
      <hr />
      <b class="title text-danger h4" >SẢN PHẨM KHUYẾN MÃI</b>
      ...
      <ul class="row list-unstyled gy-3 mt-3" >
        <li class="col-3 mb-4" v-for="sp in paginatedSanPhams" :key="sp.id">
          <div class="card h-100 position-relative" >
            ...
            <div v-if="sp.sp_khuyenmai>0" class="badge bg-warning text-dark position-absolute" style="top: -10px; right: -10px;">
              Giảm {{sp.sp_khuyenmai}}%
            </div>

            <div class="card-body d-flex flex-column justify-content-between">
              <div class="text-center">
                <p class="mb-1 h5 text-decoration-line-through">{{ formatCurrency(sp.sp_dongia) }}</p>
                <p class="mb-2 h3 text-danger">{{ formatCurrency(sp.sp_dongia-sp.sp_dongia*sp.sp_khuyenmai/100) }}</p>
              </div>
...
export default {
  ...
  methods: {
    ...
    dssanpham() {
      axios.get('http://localhost:8000/khuyenmai')
        .then((response) => {
