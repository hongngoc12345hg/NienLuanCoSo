<template>
  <div class="container-fluid min-vh-100 d-flex justify-content-center align-items-center bg-light text-dark">
    <div class="col-12 col-md-8 col-lg-6 bodyform p-4 rounded-4 shadow">
      <h3 class="text-center mb-4 fw-bold text-golden">Thông tin sản phẩm</h3>
      <form class="row g-3">
        <div class="col-md-6">
        ...
        <div class="col-12">
          ...
          <input type="number" class="form-control form-control-lg border-golden" placeholder="Khuyến mãi:" v-model="sp_khuyenmai">
        </div>
        <div class="col-md-6">
        ...
        <div class="col-md-6">
          ...
          <div class="imgsp position-relative">
            <img class="hinhanh" :src="sp_duongdan" alt="">
        ...
        <div class="col-12 text-center">
          <button type="button" class="btn btn-golden w-100 btn-lg shadow-sm submit" @click="luusanpham()">Lưu</button>
        </div>
...
export default {
  ...
  methods: {
    ...
    imgchange(e){
      ...
      this.sp_hinhanh = file
      this.sp_duongdan = URL.createObjectURL(file)
    },
    suasanpham(){
      const formData = {};
      formData._id=this.sp_id
      formData.sp_ten=this.sp_ten
      formData.sp_mota=this.sp_mota
      formData.sp_soluong=this.sp_soluong
      formData.sp_dongia=this.sp_dongia
      formData.sp_namsx=this.sp_namsx
      formData.nsx_ma=this.nsx_ma
      formData.loai_ma=this.loai_ma
      formData.sp_khuyenmai=this.sp_khuyenmai
      axios.post('http://localhost:8000/sanpham/suasp', formData)
    ...
    async exampleWithDelay() {
        console.log("Bắt đầu");
        await delay(5000); // delay 2 giây
        console.log("2 giây sau");
    },
    async luusanpham(){
        if(this.sp_hinhanh=== null){       
            this.suasanpham();  
        }
        else{
            const formData = new FormData();
            formData.append('_id',this.sp_id)
            formData.append('file',this.sp_hinhanh)
            axios.post('http://localhost:8000/sanpham/suaanh', formData)
                .then(function (response) {
                })
                .catch(function (error) {
                console.log(error);
        ...
        this.suasanpham();
        this.$router.replace({ path: '/HienthiSP' }).then(() => {
  ...
  created(){
    ...
    axios.get('http://localhost:8000/sanpham/'+id)
        .then((response) => {
            console.log(response.data)
